...
public class WPTask {
	...
	return runIt(
		String.format(cmdTmplt ,  content.getAbsolutePath())
	);
    }
	...
	return runIt(
		String.format(cmdTmplt, imagePath, articleID)
	);
    }
	...
	metaMap.entrySet().forEach(entry -> {
	    try {
		runIt(
			String.format(cmdTmplt, aid, shellScape(entry.getKey()), shellScape(entry.getValue()))
		);
	    } catch (Exception e) {
		e.printStackTrace();
	    }
